...
gs_nat_sci_all$Type <- rep("Genomic Selection Papers", nrow(gs_nat_sci_all))
dim(gs_nat_sci_all)
...
gs_other_all$Type <- rep("Genomic Selection Papers", nrow(gs_other_all))
dim(gs_other_all)
...
yield_nat_sci_all$Type <- rep("Transgenic & Yield Papers", nrow(yield_nat_sci_all))
...
yield_other_all$Type <- rep("Transgenic & Yield Papers", nrow(yield_other_all))
dim(yield_other_all)
...
# Compute percentages
nature_trends$fraction <- nature_trends$n / sum(nature_trends$n)
other_trends$fraction <- other_trends$n / sum(other_trends$n)
...
a <- ggplot(nature_trends, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Type)) +
  geom_rect() +
  ...
  ggtitle("#WOS papers for Nature and Science")


b <- ggplot(other_trends, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Type)) +
  geom_rect() +
  geom_text( x=2, aes(y=labelPosition, label=label), size=6) + # x here controls label position (inner / outer)
  scale_fill_manual(values = ncp) +
  scale_color_manual(values = ncp) +
...
ggsave(filename = "~/git_projects/p_yield/images/gs_vs_transgenic_nat_sci_vs_all_others.png",
